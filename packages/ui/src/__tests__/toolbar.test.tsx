...
import { LexifyContext } from "@lexify/react";
...
function EditorProvider({
  ...
  return (
    <LexifyContext.Provider value={editor}>{children}</LexifyContext.Provider>
  );
}
...
describe.each([
  ["BoldButton", BoldButton, "Bold", "lexify:format:bold"],
  ["ItalicButton", ItalicButton, "Italic", "lexify:format:italic"],
  ["UnderlineButton", UnderlineButton, "Underline", "lexify:format:underline"],
  [
    "StrikethroughButton",
    StrikethroughButton,
    "Strikethrough",
  ...
  ["CodeButton", CodeButton, "Code", "lexify:format:code"],
...
describe("HeadingSelect", () => {
  ...
  it("dispatches SET_HEADING_COMMAND when heading selected", () => {
    ...
    expect(editor.dispatchCommand).toHaveBeenCalledWith(
      expect.objectContaining({ type: "lexify:block:heading" }),
      "h2",
    );
  });
  ...
  it("dispatches REMOVE_HEADING_COMMAND when paragraph selected", () => {
    ...
    expect(editor.dispatchCommand).toHaveBeenCalledWith(
      expect.objectContaining({ type: "lexify:block:heading:remove" }),
      undefined,
    );
  });
  ...
  it("calls onChange callback", () => {
    ...
    render(
      <EditorProvider editor={editor}>
        <HeadingSelect onChange={onChange} />
      </EditorProvider>,
    );
...
describe("TextAlignButton", () => {
  ...
  it("dispatches SET_TEXT_ALIGN_COMMAND with correct align value", () => {
    ...
    fireEvent.click(screen.getByRole("button"));
    expect(editor.dispatchCommand).toHaveBeenCalledWith(
      expect.objectContaining({ type: "lexify:block:text-align" }),
      "right",
    );
  });
});
...
import {
  ...
  NumberedListButton,
  RemoveListButton,
} from "../components/ListButtons.js";
...
describe("BulletListButton", () => {
  ...
  it("dispatches INSERT_UNORDERED_LIST_COMMAND", () => {
    ...
    fireEvent.click(screen.getByRole("button"));
    expect(editor.dispatchCommand).toHaveBeenCalledWith(
      expect.objectContaining({ type: "lexify:list:insert:bullet" }),
      undefined,
    );
  });
});
...
describe("NumberedListButton", () => {
  ...
  it("dispatches INSERT_ORDERED_LIST_COMMAND", () => {
    ...
    fireEvent.click(screen.getByRole("button"));
    expect(editor.dispatchCommand).toHaveBeenCalledWith(
      expect.objectContaining({ type: "lexify:list:insert:number" }),
      undefined,
    );
  });
});
...
describe("RemoveListButton", () => {
  ...
  it("dispatches REMOVE_LIST_COMMAND", () => {
    ...
    fireEvent.click(screen.getByRole("button"));
    expect(editor.dispatchCommand).toHaveBeenCalledWith(
      expect.objectContaining({ type: "lexify:list:remove" }),
      undefined,
    );
  });
});
...
describe("FontSizeInput", () => {
  ...
  it("dispatches SET_FONT_SIZE_COMMAND on blur with valid value", () => {
    ...
    expect(editor.dispatchCommand).toHaveBeenCalledWith(
      expect.objectContaining({ type: "lexify:style:font-size" }),
      "16px",
    );
  });
  ...
  it("dispatches REMOVE_FONT_SIZE_COMMAND when cleared", () => {
    ...
    fireEvent.blur(input);
    expect(editor.dispatchCommand).toHaveBeenCalledWith(
      expect.objectContaining({ type: "lexify:style:font-size:remove" }),
      undefined,
    );
  });
  ...
  it("calls onInvalidValue and does NOT dispatch for invalid value", () => {
    ...
    render(
      <EditorProvider editor={editor}>
        <FontSizeInput onInvalidValue={onInvalidValue} />
      </EditorProvider>,
    );
    const input = screen.getByRole("textbox");
...
describe("FontColorInput", () => {
  ...
  it("dispatches SET_FONT_COLOR_COMMAND on blur with valid color", () => {
    ...
    fireEvent.blur(input);
    expect(editor.dispatchCommand).toHaveBeenCalledWith(
      expect.objectContaining({ type: "lexify:style:font-color" }),
      "#ff0000",
    );
  });
  ...
  it("dispatches REMOVE_FONT_COLOR_COMMAND when cleared", () => {
    ...
    fireEvent.blur(input);
    expect(editor.dispatchCommand).toHaveBeenCalledWith(
      expect.objectContaining({ type: "lexify:style:font-color:remove" }),
      undefined,
    );
  });
  ...
  it("calls onInvalidValue and does NOT dispatch for invalid color", () => {
    ...
    render(
      <EditorProvider editor={editor}>
        <FontColorInput onInvalidValue={onInvalidValue} />
      </EditorProvider>,
    );
    const input = screen.getByRole("textbox");
...
describe("UndoButton", () => {
  ...
  it("dispatches UNDO_COMMAND on click", () => {
    ...
    expect(editor.dispatchCommand).toHaveBeenCalledWith(
      expect.objectContaining({ type: "lexify:history:undo" }),
      undefined,
    );
  });
  ...
  it("accepts a custom aria-label", () => {
    render(
      <EditorProvider editor={editor}>
        <UndoButton aria-label="Step back" />
      </EditorProvider>,
    );
...
describe("RedoButton", () => {
  ...
  it("dispatches REDO_COMMAND on click", () => {
    ...
    expect(editor.dispatchCommand).toHaveBeenCalledWith(
      expect.objectContaining({ type: "lexify:history:redo" }),
      undefined,
    );
  });
  ...
  it("accepts a custom aria-label", () => {
    render(
      <EditorProvider editor={editor}>
        <RedoButton aria-label="Step forward" />
      </EditorProvider>,
    );

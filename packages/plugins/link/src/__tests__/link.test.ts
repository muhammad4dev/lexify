...
describe("linkPlugin", () => {
  it("has the correct name", () => {
    expect(linkPlugin.name).toBe("lexra/link");
  });
  ...
  it("registers handlers for all three commands on mount", () => {
    ...
    expect(registeredTypes).toContain("lexra:link:insert");
    expect(registeredTypes).toContain("lexra:link:update");
    expect(registeredTypes).toContain("lexra:link:remove");
  });
  ...
  it("calls update for UPDATE_LINK_COMMAND", () => {
    const editor = createEditor({ namespace: "test", plugins: [linkPlugin] });
    const updateSpy = vi.spyOn(editor, "update");
    editor.dispatchCommand(UPDATE_LINK_COMMAND, { url: "https://new.com", target: "_blank" });
    expect(updateSpy).toHaveBeenCalledOnce();
  });
...
describe("command type strings", () => {
  it("INSERT_LINK_COMMAND type", () => {
    expect(INSERT_LINK_COMMAND.type).toBe("lexra:link:insert");
  });
  it("UPDATE_LINK_COMMAND type", () => {
    expect(UPDATE_LINK_COMMAND.type).toBe("lexra:link:update");
  });
  it("REMOVE_LINK_COMMAND type", () => {
    expect(REMOVE_LINK_COMMAND.type).toBe("lexra:link:remove");
  });
});

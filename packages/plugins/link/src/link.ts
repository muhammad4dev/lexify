...
export const INSERT_LINK_COMMAND: LexraCommand<LinkPayload> =
  createCommand<LinkPayload>("lexra:link:insert");

export const UPDATE_LINK_COMMAND: LexraCommand<LinkPayload> =
  createCommand<LinkPayload>("lexra:link:update");

export const REMOVE_LINK_COMMAND: LexraCommand<void> =
  createCommand<void>("lexra:link:remove");

// ─── Plugin ───────────────────────────────────────────────────────────────────

export const linkPlugin: LexraPlugin = {
  name: "lexra/link",
  nodes: [LinkNode],
        ...
        editor.update(() => {
          ...
          for (const node of nodes) {
            ...
            if ($isLinkNode(parent)) {
              ...
              if (payload.target !== undefined) parent.setTarget(payload.target);
